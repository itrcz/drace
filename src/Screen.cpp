...
Screen::Screen() : LCD(LCD_SDA, LCD_SCK, LCD_A0, LCD_RST, LCD_CS) {
...
void Screen::PageLoad(uint8_t pageId) {
    if (currentPage >= 0) {
        this->PageUnload();
    }
    currentPage = pageId;
...
/*
 ...
 Boot
 */
void Screen::Splash_load(void) {
    LCD.clear();
    LCD.drawXBitmap(0, 0, boot_img_bits, boot_img_width, boot_img_height, BLACK);
    LCD.update();
}
void Screen::Splash_unload(void) {}
void Screen::Splash_update(void) {}
...
void Screen::Menu_unload(void) {
    
}
void Screen::Menu_update(void) {
...
void Screen::Accel_unload() {
    
}

void Screen::Accel_update() {

    LCD.update();
}


void Screen::Accel_update_sat( void ) {
...
void Screen::Accel_update_sat_message() {
...
void Screen::Accel_update_race_message( bool clear ) {
    if (currentPage != page_accel) {
        return;
    }
    if (clear) {
...
void Screen::Accel_update_speed( void ) {
    if (currentPage != page_accel) {
        return;
    }
...
void Screen::Accel_update_results_table(bool update) {
...
void Screen::Accel_results_table_switch(bool next) {
...
void Screen::Debug_unload(void) {
    gps.debug = false;
    
}


void Screen::handleButtons( void ) {
    switch (currentPage) {
        case page_menu:
            if (ButtonRight.allow()) Menu_next();
            if (ButtonLeft.allow())  Menu_prev();
            if (ButtonMenu.allow())  Menu_select();
            if (ButtonReset.allow()) {}
            break;
        case page_accel:
            if (ButtonRight.allow()) this->Accel_results_table_switch(true);
            if (ButtonLeft.allow())  this->Accel_results_table_switch(false);
            if (ButtonMenu.allow())  this->PageLoad(page_menu);
            if (ButtonReset.allow()) this->Accel_reset();
            break;
        case page_debug:
            if (ButtonRight.allow()) {}//Menu_next();
            if (ButtonLeft.allow())  {}//Menu_prev();
...
void Screen::Update( void ) {
    ...
    if (!timeHalfSecond) {
        timeHalfSecond = millis();
    }
    if (millis() - timeHalfSecond > 500) {
        timeHalfSecond = millis();
        boolHalfSecond = !boolHalfSecond;
    }
};
